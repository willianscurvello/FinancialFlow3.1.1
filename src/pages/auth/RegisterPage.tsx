...
export function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    try {
      ...
      if (!userId) {
        console.error("User ID is undefined after sign-up.");
        return; // Evita tentar inserir um usuário sem ID
      }

      // Agora, insira na tabela users
      const { error: insertError } = await supabase
        .from('users')
        .insert([{ email: data.email, id: userId, created_at: new Date() }]); // Adicione outros campos conforme necessário

      if (insertError) {
        console.error('Insert error:', insertError);
      } else {
        console.log('User successfully inserted:', insertError);
      }
